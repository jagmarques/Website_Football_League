				...
				<div id="firstboxText">
                    ...
                    try{
                        ...
                        if ($connection->connect_errno != 0){
        					...
        					$sql ="SELECT * FROM game WHERE state=0 and contest_contest_name = '$contest';";
        					if($sql!=NULL){
            					$result = mysqli_query($connection,$sql);
            					$resultCheck = mysqli_num_rows($result);
            					if($resultCheck>0){
                					while($row = mysqli_fetch_assoc($result)){
										echo '<p>'.$row['team_team_name'] . ' VS ' .$row['team_team_name1'] .' Date: ' .$row['game_date']. 'Hour:'.$row['game_hour'].'pm</p><input type="submit" name="editgame"></button><br><br><br>';
									}
            					}
                                else{
